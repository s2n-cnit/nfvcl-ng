...
from nfvcl.models.blueprint_ng.k8s.k8s_rest_models import K8sCreateModel, K8sAreaDeployment, K8sAddNodeModel, KarmadaInstallModel, K8sDelNodeModel, CreateVxLanModel
...
class K8sBlueprintNGState(BlueprintNGState):
    ...
    pod_service_cidr: str = Field(default=POD_SERVICE_CIDR, description="The internal network used for Services by the cluster")
...
class K8sBlueprint(BlueprintNG[K8sBlueprintNGState, K8sCreateModel]):
    ...
    def day0conf(self, topology_onboard: bool, configure_master: bool = False):
        ...
        if topology_onboard:
            ...
            k8s_cluster = K8sModel(name=self.id, provided_by="NFVCL", blueprint_ref=self.id, credentials=self.state.master_credentials, vim_name=topo.get_vim_name_from_area_id(self.state.master_area.area_id),  # For the constraint on the model, there is always a master area.
                                   k8s_version=K8sVersion.V1_29, networks=self.state.attached_networks, areas=area_list, cni="", nfvo_onboard=False, anti_spoofing_enabled=not self.state.require_port_security_disabled)
    ...
    def install_default_plugins(self):
        ...
        add_data = K8sTemplateFillData(pod_network_cidr=pod_network_cidr, lb_ipaddresses=workers_mgt_int, lb_ipaddresses_auto=self.state.load_balancer_ips,
