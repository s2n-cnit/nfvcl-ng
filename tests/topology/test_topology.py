...
from tests.topology.models import TOPOLOGY_OK, VIM_TO_ADD1, VIM_TO_ADD2, PDU_TO_ADD, K8S_TO_ADD, PROMETHEUS_TO_ADD
...
class TestGroupTopology(NFVCLTestSuite):
    ...
    def test_get_vim(self, topology_context: TopologyTestContext):
        vim = topology_context.topology_model.vims[0]
        assert self.nfvcl.get_vim(vim.name) == vim

    def test_create_vim(self, topology_context: TopologyTestContext):
    ...
    def test_update_vim(self, topology_context: TopologyTestContext):
        vim = copy.deepcopy(topology_context.vim_to_add2_model)
        vim.areas = [10,11]
        self.nfvcl.update_vim(vim)
        assert self.nfvcl.get_vim(vim.name) == vim

    def test_delete_vim(self, topology_context: TopologyTestContext):
    ...
    def test_get_pdus_empty(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_pdus() == []

    def test_create_pdu(self, topology_context: TopologyTestContext):
        self.nfvcl.create_pdu(topology_context.pdu_model)
        assert self.nfvcl.get_pdus() == [topology_context.pdu_model]

    def test_get_pdu(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_pdu(topology_context.pdu_model.name) == topology_context.pdu_model

    def test_delete_pdu(self, topology_context: TopologyTestContext):
    ...
    def test_get_kubernetes_empty(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_kubernetes_list() == []

    def test_create_kubernetes(self, topology_context: TopologyTestContext):
    ...
    def test_get_kubernetes(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_kubernetes(topology_context.kubernetes_model.name) == topology_context.kubernetes_model

    def test_delete_kubernetes(self, topology_context: TopologyTestContext):
        self.nfvcl.delete_kubernetes(topology_context.kubernetes_model.name)

        with pytest.raises(Exception):  # noqa: B017
            self.nfvcl.delete_kubernetes("non-existing")

    def test_get_prometheus_empty(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_prometheus_list() == []

    def test_create_prometheus(self, topology_context: TopologyTestContext):
    ...
    def test_get_prometheus(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_prometheus(topology_context.prometheus_model.id) == topology_context.prometheus_model

    def test_update_prometheus(self, topology_context: TopologyTestContext):
    ...
    def test_back_to_original(self, topology_context: TopologyTestContext):
        assert self.nfvcl.get_topology() == topology_context.topology_model
