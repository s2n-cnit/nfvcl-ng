...
from nfvcl.blueprints_ng.modules.generic_5g.generic_5g_upf import DeployedUPFInfo, Router5GInfo
...
from nfvcl.blueprints_ng.modules.open5gs import open5gs_default_upf_k8s_config
...
OPEN5GS_UPF_K8S_BLUE_TYPE = "open5gs_upf_k8s"


class Open5GsUPFK8sBlueprintNGState(Generic5GUPFK8SBlueprintNGState):
    upf_conf: Dict[str, Open5GsUpfK8sConfig] = Field(default_factory=dict)
...
class Open5GsUPFK8SBlueprintNG(Generic5GUPFK8SBlueprintNG[Open5GsUPFK8sBlueprintNGState, UPFBlueCreateModel]):
    ...
    def spawn_upf(self, dnn: str):
        self.logger.info(f"Starting creation of Open5GsK8sBlueprintNG blueprint for dnn: {dnn}")
        if dnn not in self.state.upf_conf:
            self.state.upf_conf[dnn] = copy.deepcopy(open5gs_default_upf_k8s_config.default_upf_config)
        ...
        if self.state.current_config.networks.n4.type == NetworkEndPointType.MULTUS:
            ...
            self.state.upf_conf[dnn].multus.n4network.set_multus(self.state.multus_network_info.n4)
        if self.state.current_config.networks.n3.type == NetworkEndPointType.MULTUS:
            ...
            self.state.upf_conf[dnn].multus.n3network.set_multus(self.state.multus_network_info.n3, self.state.current_config.n3_gateway_ip.exploded)
        if self.state.current_config.networks.n6.type == NetworkEndPointType.MULTUS:
            ...
            self.state.upf_conf[dnn].multus.n6network.set_multus(self.state.multus_network_info.n6, self.state.current_config.n6_gateway_ip.exploded)

        self.state.upf_conf[dnn].config.upf.pfcp.address = self.state.multus_network_info.n4.ip_address.exploded
        self.state.upf_conf[dnn].config.upf.pfcp.node_id = self.state.multus_network_info.n4.ip_address.exploded
        self.state.upf_conf[dnn].config.upf.gtpu.address = self.state.multus_network_info.n3.ip_address.exploded
        self.state.upf_conf[dnn].config.upf.gtpu.address = self.state.multus_network_info.n3.ip_address.exploded
        self.state.upf_conf[dnn].config.upf.gnb_subnet = self.state.current_config.gnb_cidr.exploded

        self.state.upf_conf[dnn].config.subnet_list.clear()

        for _slice in self.state.current_config.slices:
            for dnnslice in _slice.dnn_list:
                if dnnslice.dnn == dnn:
                    self.state.upf_conf[dnn].config.add_session(
                        dnn=dnn,
                        cidr=dnnslice.cidr
                    )
        ...
        self.provider.install_helm_chart(upf_helm_chart, self.state.upf_conf[dnn].model_dump(exclude_none=True, by_alias=True))
    ...
    def stop_upf(self, dnn: str):
        self.logger.info(f"Stopping Open5GsK8sBlueprintNG blueprint for dnn: {dnn}")
        ...
        self.logger.info(f"Open5GsK8sBlueprintNG blueprint dnn: {dnn} stopped")

        return upf_info

    def update_upf(self):
        self.logger.debug("Open5GsBlueprintNG update")
        self.update_deployments()
